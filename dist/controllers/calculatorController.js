...
exports.generatePDF = exports.getCalculationById = exports.getCalculationHistory = exports.generateDivisionPDF = exports.calculateDivisionHazard = void 0;
...
const calculatorResult_1 = __importDefault(require("../models/calculatorResult"));
const puppeteer_1 = __importDefault(require("puppeteer"));
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const getRiskLevel = (averageScore) => {
    if (averageScore >= 0 && averageScore < 0.25)
        return 'Severely High Risk';
    else if (averageScore >= 0.25 && averageScore < 0.5)
        return 'High Risk';
    else if (averageScore >= 0.5 && averageScore < 0.75)
        return 'Moderate Risk';
    else if (averageScore >= 0.75 && averageScore < 0.9)
        return 'Low Risk';
    else
        return 'Very Low Risk';
};
const calculateDivisionHazard = async (req, res) => {
    try {
        const { division } = req.params;
        console.log(`Calculating for division: ${division}`);
        const entries = await entryModel_1.default.find({ division });
        ...
        for (const entry of entries) {
            const criteria = await criteriaModel_1.default.find({ entryId: entry._id });
            ...
            for (const criterion of criteria) {
                const config = await configModel_1.default.findOne({ value: criterion.weight });
...
exports.calculateDivisionHazard = calculateDivisionHazard;
const generateDivisionPDF = async (req, res) => {
    try {
      ...
      <body>
        ...
        <div class="entry-section">
          ...
          ${divisionData.entryResults.map((entry) => `
            <div class="entry-card">
              <h3>
                <span>${entry.climateHazardCategory}</span>
                <span class="risk-badge ${entry.riskLevel.includes('High') ? 'risk-high' :
            entry.riskLevel.includes('Moderate') ? 'risk-moderate' :
                entry.riskLevel === 'No Data' ? 'risk-no-data' : 'risk-low'}">${entry.riskLevel}</span>
              </h3>
        ...
        <div class="division-result">
          ...
          <div class="final-risk ${divisionData.divisionRiskLevel.includes('High') ? 'high' :
...
exports.generateDivisionPDF = generateDivisionPDF;
const getCalculationHistory = async (req, res) => {
    try {
        const history = await calculatorResult_1.default.find().populate('entryId').sort({ calculatedDate: -1 });
...
const getCalculationById = async (req, res) => {
    try {
        const { id } = req.params;
        const calculation = await calculatorResult_1.default.findById(id).populate('entryId');
        if (!calculation) {
